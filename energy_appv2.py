...
gen_power_input = st.sidebar.number_input("Генераторная мощность, руб/Мвт", value=default_gen)
gen_change_input = st.sidebar.number_input("Ставка за управление, руб/Мвт", value=default_admin)
network_rate_input = st.sidebar.number_input("Ставка за содержание сетей, руб/Мвт", value=default_net)
...
def calculate_network_charge_average(df_scenario, current_biz_mask, hr_cols):
    ...
    if biz_data.empty or not ALL_ASSESS: return 0.0, 0.0
...
def calculate_total_energy_cost(df_scenario, price_map, hour_columns):
    ...
    for idx, row in df_scenario.iterrows():
        ...
        if day_num in price_map:
            ...
            day_cost = sum(float(row[hr]) * (float(day_data[hr]) / 1000) for hr in hour_columns)
            total_rubles += day_cost
    return round(total_rubles, 2)

# --- UPLOAD LOGIC ---
u_input = st.file_uploader("Выбрать файл (xlsx)", type=["xlsx"])
try:
    ...
    st.error(f"Ошибка тарифов: {e}"); st.stop()
...
if u_input:
    if st.button("🚀 Симулируем"):
        with st.spinner("Рассчитываем..."):
            ...
            if target_mask_list is None: st.stop()
            ...
            for m in MODULE_COUNTS:
                ...
                for idx, row in df_raw.iterrows():
                    ...
                    morn_hrs = [h for h in ALL_ASSESS if h < (min(DYN_GAP_WINDOW) if DYN_GAP_WINDOW else 13)]
                    d_morn = optimize_discharge(row[hr_cols].values, target_mask_list[idx], cap, morn_hrs)
                    row_rem = row[hr_cols].values - np.array([d_morn[h] for h in range(24)])
                    eve_hrs = [h for h in ALL_ASSESS if h not in morn_hrs]
                    d_eve = optimize_discharge(row_rem, target_mask_list[idx], cap, eve_hrs)
                    net_flow = {h: min(cap, d_morn[h] + d_eve[h]) for h in range(24)}
                    
                    for window, vol_t in [(DYN_NIGHT_WINDOW, "night"), (DYN_GAP_WINDOW, "gap")]:                        
                        ...
                        for h in cheapest:
                            ...
                            net_flow[h] -= charge 
                            needed -= charge
                            if vol_t == "night": total_night_charge_vol += charge
                            else: total_gap_charge_vol += charge

                    for h in range(24):
                        b_val = float(row[hr_cols[h]])
                        f_val = float(net_flow[h])
                        df_sim.at[idx, hr_cols[h]] = max(0.0, b_val - f_val)
                        df_schedule.at[idx, hr_cols[h]] = f_val
                ...
                summary_results.append({
                    "Setup": module_names[m], "Total Monthly kWh": round(total_kwh_sim, 2),
                    "Added from Battery": round(total_kwh_sim - total_kwh_baseline, 2),
                    "Generating Peak (kW)": round(m_gen_p, 4), "Avg Assessment Peak (MW)": m_peak_mw, 
            ...
            # EXECUTIVE REPORT
            def get_avg_p(res):
                return res['Total Consumption Cost'] / (res['Total Monthly kWh'] * KW_TO_MWH) if res['Total Monthly kWh'] > 0 else 0
            ...
            v_report = [
                ...
                {"": "Средняя стоимость электроэнергии, руб/MВтч", **{res['Setup']: round(get_avg_p(res), 2) for res in summary_results}},
                {"": "Генераторная мощность, руб/МВт", **{res['Setup']: round(TOTAL_RATE_RUB_M_WH, 2) for res in summary_results}},
                {"": "Сетевая мощность, руб/МВт", **{res['Setup']: round(NETWORK_CAPACITY_RATE, 2) for res in summary_results}},
                {"": "", "ФАКТ": "", "5_Modules 73kW": "", "6_Modules 87,6kW": "", "7_Modules 102,2kW": "", "8_Modules 116,8kW": ""},
            ...
            st.download_button("📥 Скачать результаты", output.getvalue(), file_name=f"results_{month_choice}.xlsx")
