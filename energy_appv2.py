...
try:
    ...
    st.error(f"Критическая ошибка: {e}"); st.stop()

st.sidebar.header("Параметры тарифов (Проверка)")
...
def optimize_discharge_aggressive(row_data, target_map, capacity, active_window):
    ...
    # Priority 2: Peak Shaving (Leveling)
    while rem > 0.0001:
...
if u_input:
    ...
    try:
        ...
        st.error(f"Assessment Hours Load Failed: {e}"); st.stop()
    ...
    if st.button("🚀 Начать расчет"):
        ...
        for config in configs:
            ...
            for i, row in df_raw.iterrows():
                ...
                total_d = morn_d + eve_d
                total_c = charge_gap + charge_night
                for h in range(24):
                    df_sim.at[i, HR_COLS[h]] = round(max(0, row[HR_COLS[h]] - total_d[h] + total_c[h]), 4)
                    df_sch.at[i, HR_COLS[h]] = round(total_d[h] - total_c[h], 4)
        ...
        # --- 5. FULL EXECUTIVE FINANCIAL REPORT ---
        ...
        with pd.ExcelWriter(out, engine='openpyxl') as writer:
            ...
            pd.DataFrame(df_raw).to_excel(writer, sheet_name="Baseline", index=False)
