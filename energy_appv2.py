...
if u_input:
    ...
    if st.button("🚀 Начать расчет"):
        ...
        v_report = [
            ...
            {"": "Средняя стоимость руб/кВтч", **{r['Setup']: round(r['Total Consumption Cost']/r['Total Monthly kWh'], 2) if r['Total Monthly kWh'] > 0 else 0 for r in results}},
            {"": "ИТОГО руб (без НДС)", **{r['Setup']: r['GRAND TOTAL COST'] for r in results}},
            {"": "ИТОГО руб (с НДС)", **{r['Setup']: round(r['GRAND TOTAL COST']*1.2, 2) for r in results}},
        ]
